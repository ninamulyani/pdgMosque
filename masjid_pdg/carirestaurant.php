<?php
    ...
    $querysearch = "SELECT id,name, address, land_size, park_area_size, building_size, capacity, est, last_renovation, jamaah, imam, teenager, id_category,
    ST_X(ST_Centroid(geom)) AS lng, ST_Y(ST_CENTROID(geom)) As lat FROM worship_place WHERE
    ST_Intersects(ST_Centroid(worship_place.geom),ST_buffer(ST_GeomFromText(concat('POINT($longi $latit)')), 0.0009*$rad))=1";
