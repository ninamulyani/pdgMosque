...
var awal = 0;
var tujuan = 0;
var server = "http://localhost/masjid_pdg/";

//cek stts radius, pastikan stts off dulu sebelum ada aksi 
...
//Fungsi geolokasi / posisi saat ini
...
function aktifkanRadius(){ 
    ...
    var circle = new google.maps.Circle({ 
        ...
        map: map,
        strokeColor: "blue",  //warna pinggir radius
        strokeOpacity: 0.8,   //transparansi pinggir
        strokeWeight: 2,      //lebar outline radius
        fillColor: "blue",    //warna isi radius
        fillOpacity: 0.35     //transparansi isi radius
      });        
      map.setZoom(14);       
      map.setCenter(pos);
      circles.push(circle);     
 ...
 function masjidradius(){ //menampilkan masjid berdasarkan radius
  ...
  $('#hasilcari1').show(); //hasilcari1 = id class result pencarian & event || hasilcari2 = id class attracions around
  $('#hasilcari').empty(); //hasilcari = id tabel
      hapusInfo();
      clearroute2();
	    clearroute();
      clearangkot();
  ...
  // fungsi menampilkan semua masjid
  ...
  // fungsi cari masjid berdasarkan nama 
  ...
  //fungsi cari event berdasarkan date/tanggal
  function caritglk(rows){   
    $('#hasilcari1').show();
    $('#hasilcari').show();
    $('#hasilcari').empty();
  ...
  function carikategori(){
    ...
    $.ajax({ 
      url: server+'carikat.php?id_kategori='+kat, data: "", dataType: 'json', success: function(rows)
        { 
          console.log("Pencarian berdasarkan kategori");
          if(rows==null)
            {
  ...
  //fungsi cari mesjid berdasarkan kendaraaan yang digunakan
  ...
  //fungsi cari mesjid berdasarkan nama
  ...
  //menampilkan Information oleh oleh/suvenir
  function detailoleh(id1){  
...
function detailik(id1){  //menampilkan Information industri kecil
...
console.log("aada");	
			...
			if (document.getElementById('check_j').checked) {
...
function hotelsekitar(latitude,longitude,rad){ // HOTEL SEKITAR MASJID
	...
	// OW SEKITAR MASJID	
	function owsekitar(latitude,longitude,rad){ 
          $('#hasilcari2').show();
		  $('#hasilcariaround').empty();
		  cekRadius1();
    ...
    // restaurant SEKITAR MASJID
    function restaurantsekitar(latitude,longitude,rad){ 
          $('#hasilcari2').show();
		  $('#hasilcariaround').empty();
      cekRadius1();
...
//fungsi cari berdasarkan wilayah dan kategori
function tampilkatwilayah(){ 
...
//tampil pilihan cari tgl untuk kegiatan
function caritglkeg(){
  if(caritgl.value==''){
    alert("Fill the input form first!");
    }else{
 ...
 function posisiawal(){ //POSISI AWAL
    ...
    $('#hasilcari').append("<thead><th>Angkot Code</th><th>Route</th><th>Gallery</th></thead>");
	  ...
	  function reset_rute () { //RESET KLIK RUTE
          ...
          basemap();          //gunakan fungsi basemap, 
		  $('#hasilcari').empty();
		  $('#hasilcari').append("<thead><th>Angkot Code</th><th>Route</th><th>Gallery</th></thead>");
