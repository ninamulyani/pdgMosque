...
function cari_f1a(){
  ...
  var link = 'k3.php?kode=1a&act=cari&ustad='+inp_ustad+'&kategori='+inp_kategori;
    console.log(link);
  $.ajax(
    { url: link, data: "", dataType: 'json', success: function (rows){
    data_f1a(rows);
  }});
}
...
function cari_f1ra() {
  var inp_kecamatan = se_kecamatan_f1_ra.children("option:selected").val(); //fungsional kecamatan dihilangkan 
    ...
    if(rows==null){
      ...
      for (var i in rows){
        ...
        marker        = new google.maps.Marker({
          position: centerBaru,
          icon:'assets/ico/marker_masjid.png',            //marker masjid
          map: map,
        ...
        map.setZoom(14); 
            // Tampilan hasil result pencarian : nama | ikon info | ikon kendaraan
            $('#hasilcari').append(
              "<tr><td>"+nama+"</td><td><a role='button' title='info' class='btn btn-default fa fa-info' onclick='detailmasjid(\""+id+"\");info1();'></a></td><td><a role='button' class='btn btn-default fa fa-bus' title='jalur angkot' onclick='angkotmesjid(\""+id+"\",\""+latitude+"\",\""+longitude+"\");info12();'></a></td></tr>");
      }
    }
}
   ...
   success: function(rows){
      for (var i in rows){
          ...
          infowindow = new google.maps.InfoWindow({
            position: centerBaru,
            //layout info window pada marker
...
function firstloc() {
  ...
  usegeolocation=true;
  google.maps.event.clearListeners(map, 'click'); //close alert current location
  console.log(first);
}

function route_to_begin() {

    var start = pos; //posisi awal
    var end = "";    //posisi akhir null karna belum ditentukan

    // Check posisi awal pada cookie 
    if (!checkCookie('location')){
    ...
    //jika belum menentukan posisi awal: current location/ manual position, posisi awal=null
    if (start == 'null'){
      alert ('Click button current position or manual position first !');
    }
